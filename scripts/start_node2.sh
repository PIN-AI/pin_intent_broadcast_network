...
NODE_ID=2
NODE_NAME="监控节点"
CONFIG_FILE="$NODE2_CONFIG_FILE"
HTTP_PORT="$NODE2_HTTP_PORT"
GRPC_PORT="$NODE2_GRPC_PORT"
P2P_PORT="$NODE2_P2P_PORT"
LOG_FILE="$NODE2_LOG_FILE"
PID_FILE="$NODE2_PID_FILE"
STATUS_FILE="$NODE2_STATUS_FILE"
...
show_node_header() {
    clear
    echo -e "${MAGENTA}================================${NC}"
    echo -e "${MAGENTA}    PIN 意图广播网络 - 节点2    ${NC}"
...
check_prerequisites() {
    ...
    # 检查测试数据目录
    if [ ! -d "$NODE2_DATA_DIR" ]; then
        log_error "节点数据目录不存在: $NODE2_DATA_DIR"
        log_error "请先运行 './scripts/setup_test_env.sh' 初始化环境"
        exit 1
    fi
...
show_node_info() {
    ...
    log_step "节点运行信息"
    echo ""
    echo -e "${GREEN}✓ 节点2 ($NODE_NAME) 运行中${NC}"
    echo ""
    echo "服务端点:"
...
start_intent_monitor() {
    ...
    if [ -f "./scripts/intent_monitor.sh" ]; then
        ...
        local monitor_pid=$!
        echo $monitor_pid > "$NODE2_DATA_DIR/monitor.pid"
    ...
    tail -f "$LOG_FILE" 2>/dev/null || {
        ...
        while true; do
            if [ -f "$PID_FILE" ]; then
                local pid=$(cat "$PID_FILE")
                if kill -0 "$pid" 2>/dev/null; then
                    echo "$(date '+%Y-%m-%d %H:%M:%S') [INFO] 节点2运行中 (PID: $pid)"
                else
                    log_error "节点进程已停止"
                    break
                fi
...
handle_interrupt() {
    echo ""
    log_step "接收到停止信号，正在关闭节点2..."
    
    # 停止 Intent 监控器
    if [ -f "$NODE2_DATA_DIR/monitor.pid" ]; then
        local monitor_pid=$(cat "$NODE2_DATA_DIR/monitor.pid")
        if kill -0 "$monitor_pid" 2>/dev/null; then
            log_info "停止 Intent 监控器 (PID: $monitor_pid)"
            kill -TERM "$monitor_pid" 2>/dev/null || true
        fi
        rm -f "$NODE2_DATA_DIR/monitor.pid"
    fi
